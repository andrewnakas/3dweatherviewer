...
export class CloudLayer {
  ...
  render(gl, matrix) {
    ...
    // world lattice: cell size from the spawn box, origin snapped to absolute
    // cell coords so panning shifts coverage without moving existing puffs
    const G = this.grid;
    const cell = [
      Math.max((spawn.max[0] - spawn.min[0]) / G, 1e-5),
      Math.max((spawn.max[1] - spawn.min[1]) / G, 1e-5),
    ];
